...
$participation_sql = "SELECT * FROM events_participation WHERE participationStatus = 'Approved' ORDER BY participationStatus DESC";
...
while ($row = $participation_result->fetch_assoc()) {
    ...
    if (array_key_exists($name, $participant_counts)) {
                    ...
                    <table class="table vm no-th-brd pro-of-month" style="border-radius:10px" id="example">
                        <thead style="background:#8E8B82;color:#FFF;border-radius:10px">
                            <tr>
                                <th>No.</th>
                                <th><center>Name</th>
                                <th><center>No. of Participants</th>
                                <th><center>Action</th>
                            </tr>
                        </thead>
                        <tbody>
                                <?php
                                $i = 1;
                                foreach ($participant_counts as $name => $count) {
